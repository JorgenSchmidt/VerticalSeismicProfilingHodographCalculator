...
int main()
{
		...
		) 
	{
		...
		printf("значение шага по профилю в 25 и больше будет некорректно. ");
		getch();
		return 0;
	}
	...
	if (FormationsCount == 1) 
	{
		...
		if (LastFormationAcousticSpeed <= 0) 
		{
			printf("\nНекорректный ввод, значение скорости распространения");
			printf("\nАВ должно быть больше или равно единице");
			getch();
			return 0;
		}
